...
import { partMap, productMap } from 'src/app/utility/product-map';
...
export class SalesOrderCreateOrderComponent implements OnInit {
    ...
    vehiclePartEnum = VehiclePartEnum;
    ...
    getPartOptions(orderTypeVal: OrderTypeEnum) {
        const vehiclePartVals = partMap.get(orderTypeVal)!;
        return vehiclePartOptions.filter((item) => vehiclePartVals.includes(item.value));
    }
